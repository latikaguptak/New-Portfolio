// Personal Information
export const personalInfo = {
  name: "Latika Gupta",
  title: "Frontend Developer",
  description: "Passionate developer with expertise in building modern web with mobile responsive. Specializing in Frontent technologies like JavaScript, Reactjs , and TypeScript.",
  social: {
    github: "https://github.com/latikaguptak",
    linkedin: "https://www.linkedin.com/in/latika-gupta-varshney/",
    twitter: "https://x.com/LatikaGupta02",
    email: "latika.gupta.108@gmail.com",
    Resume:"https://lavender-elisa-74.tiiny.site"
  }
};

// Skills data
export const skills = [
  {
    category: "Frontend Development",
    items: ["React", "TypeScript","JavaScript", "HTML5", "CSS3", "Tailwind CSS", "Redux", "React Recoil"]
  },
  // {
  //   category: "Mobile Development",
  //   items: ["React Native", "Expo", "iOS", "Android", "Mobile UI", "App Store"]
  // }
];


export const experience = [
  {
    title: "Frontend Developer",
    company: "Tata Consultancy Services",
    period: "2022 - Present",
    achievements: [
      "Developed a responsive web application compatible with all major browsers and devices, leveraging React.js, JavaScript, and CSS.Optimized the user interface for a seamless, mobile-first experience, ensuring cross-browser compatibility and increasing user engagement by 20%.",
      "Integrated frontend with REST-APIs to display real-time analytical data using React-ECharts and tables, enhancing user experience with visually appealing and interactive features. Ensured smooth data flow and up-to-date content for seamless user interaction.",
      "Developed a responsive web application using React and Redux , optimizing performance through code splitting and lazy loading, which improved page load times by 60% and increased mobile user retention by 25%."
    ],
    technologies: [
      "React",
      "TypeScript",
      "JavaScript",
      "HTML5",
      "CSS3",
      "Tailwind CSS",
      "Redux",
      "React Recoil",
      "React-ECharts",
      "React-Table",
      "React-Router",
      "React Navigation",
      "Lazy Loading"

    ]
  },
 
];

// Education data
export const education = [
  
  {
    degree: "Bachelor of Technology",
    school: "Dr. A. P. J. Abdul Kalam Technical University",
    period: "2018 - 2022",
    description: "Focus on Computer system and engineering of the software developent. Learn the different programming languages and the basic of computer system. Learn the Database Management, Data Structures and Algorithm, Web Development, Operating System and different programming Languages like Python, C, JavaScript, C++ etc."
  }
];


export const projects = [
  {
    id:1,
    title: "Snap Style",
    description: "This application is a modern wardrobe management tool built with React.js and TypeScript. It allows users to organize their clothing and accessories digitally, plan outfits, and curate personal styles.",
    image: "/snapstyle.png",
    tags: ["React JS", "ContextApi", "React-Router", "Firebase"],
    githubUrl: "https://github.com/latikaguptak/SnapStyle",
    liveUrl: "https://snap-style-topaz.vercel.app",
    features: [
      "Digital Wardrobe Organization",
      "Outfit Planning and Scheduling",
      "Personal Style Curation and Suggestions",
      "Closet Inventory Management",
      "Social Features for Style Sharing and Inspiration"
    ],
    technologies: [
      
      "React js",
      "React-Router",
      "React-Recoil",
      "React-DND",
      "React-Hot-Toast",
      "Zod",
      "Tailwind CSS",
      "LocalStorage",
      "TypeScript",
      "Vite"
    ],
    role: "Frontend Developer",
    duration: "6 months",
    team: ["Latika Gupta", "Aman Gupta"]
  },
  {
    id:2,
    title: "Lunar-Flow",
    description: "LunarFlow is a modern, feature-rich web application designed specifically for women to effortlessly track and manage their menstrual cycles. The platform empowers users with tools to log period dates, predict upcoming cycles, monitor health trends, and receive personalized insights—all in a secure and user-friendly environment. Built with cutting-edge technologies, LunaFlow ensures a seamless and interactive experience tailored to meet the unique needs of every woman.",
    image: "https://images.unsplash.com/photo-1472851294608-062f824d29cc?auto=format&fit=crop&q=80&w=1000",
    tags: ["React", "TypeScript", "React-Recoil", "Next-Js"],
    githubUrl: "https://github.com/latikaguptak/Lunar-Flow.git",
    liveUrl: "https://lunar-flow.vercel.app/",
    features: [
      "Cycle Tracking and Prediction",
      "Insights and Analytics",
      "Health and Wellness Resources",
      "Accessible navigation with keyboard and screen-reader support",
      "Customizable Themes",
    ],
    technologies: [
      "React",
      "Next-js",
      "TypeScript",
      "React-Recoil",
      "tailwindcss",
      "zod",
      "recharts",
      "Animation",
      "sonner",
      "date-fns"
    ], 

    role: "Frontend Developer",
    duration: "8 months",
    team: ["Latika Gupta", "Aman Gupta"]
  },
  
];